...
4. **Slim installer via bind mount** — the installer `.bin` at the repo
   root is bind-mounted read-only at `/tmp/installer.bin`, self-extracted
   (`--keep --noexec --target`) inside the install RUN, and its temp
   files removed in the same layer — nothing installer-related is ever
   copied into a layer. xsetup downloads selected modules during build
   using the token from the secret mount.
