...
- [x] 2.3 Implement stateful output filter (INFO/WARNING suppression,
      continuation handling, `elfws.yaml` per-command reload with safe
      fallback on parse failure)
- [x] 2.4 Implement health sampler (10 s procfs sweep: descendants,
      CPU %, RSS, last PTY read) and graceful-shutdown socket request
      (refuse while busy)
- [x] 2.5 Implement `run xsct|xsdb|bootgen|dtc` operations: metadata
...
- [x] 5.2 Implement GUI profile detection (wsl/wayland/x11,
...
- [x] 6.1 Remove `ROSETTA` handling from `scripts/run.vivado.sh`
      (keep one-shot behavior otherwise)
- [x] 6.2 Remove Apple Silicon/Rosetta content from `README.md`;
      re-describe the udev stub as universal Vivado-in-Docker
      mitigation (Dockerfile comments included)

## 7. Documentation

- [x] 7.1 README: daVit usage (verbs, `.dv/` session root, exit codes,
...
- [x] 7.2 Update `AGENTS.md` links/constraints if affected; add
